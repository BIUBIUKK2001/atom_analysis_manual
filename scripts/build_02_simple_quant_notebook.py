...
- Task 3：重新选择 ROI 后的 class group 几何中心分析。
...
这里选择 notebook02 的初始 ROI 集合 `ROIS`，用于 Task 1A/1B/2 的默认空间范围和 basis 选择检查。Task 3 后面会使用独立的 `TASK3_ROIS`，不会覆盖这里的 `ROIS`。
...
# 例如 ('a',) 或 ('roi_0_a',)。翻转方向不会改变 period 长度。
FLIP_BASIS_NAMES = ()
...
fig, image_ax, side_ax = plot_measurement_segments_on_image(
    ...
    show_side_panel=True,
    show_axes=False,
    title='Task 1A a/b period segments',
)
figures['task1A_period_segment_overlay'] = fig
display(fig)
plt.close(fig)

# 长度直方图默认使用 Å；如果没有像素标定，会自动回退到 px。
length_hist_figures = plot_period_length_histograms(period_segment_table, style=FIG_STYLE)

# 角度偏差直方图使用 wrapped angle_delta_deg，避免 179°/-179° 周期跳变影响统计。
angle_hist_figures = plot_period_angle_delta_histograms(period_segment_table, style=FIG_STYLE)
...
TASK1B_MANUAL_REFERENCE_PX = {
    ...
    # 'roi_0': {'a_ref_px': 10.0, 'b_ref_px': 10.0},
}
...
这里绘制 polygon cell map。每个 valid cell 作为一个四边形叠加在原始图像上，颜色表示 `eps_a`、`eps_b`、`eps_area` 或 `area_local`。invalid cell 不绘制，边缘缺失区域保持透明。
""",
        ),
        code(
            "task1b-figures",
...
# eps_a、eps_b、eps_area 是相对 reference 的应变；area_local 是局域晶胞面积。
for value_column in ('eps_a', 'eps_b', 'eps_area', 'area_local'):
    fig, ax = plot_polygon_cell_map(
        ...
        rois=ROIS,
        style=FIG_STYLE,
        title=f'Task 1B {value_column} polygon map',
    )
    figures[f'task1B_{value_column}_polygon_map'] = fig
    display(fig)
    plt.close(fig)
""",
        ),
...
# {'default': None} 表示每个 ROI 默认使用全部 class；也可以写 {'roi_0': (0,)}。
...
# projection_axis is used to separate line/row groups, usually the normal direction of the lines/rows, not the line extension direction.
...
""",
        ...
        md(
            "task3-config-md",
            """
## 15. Task 3 config: new ROI and class groups

Task 3 使用独立的 `TASK3_ROIS`，允许你重新选择一组 ROI。它只做 ROI-level 的 unweighted geometric centroid 和 group-pair displacement，不做 cell-level 归属，也不自动解释为 charge center 或 polarization。
""",
        ),
        code(
            "task3-config",
            """
# TASK3_OPEN_ROI_PICKER：True 时重新打开 napari 选择 Task 3 专用 ROI。
# Task 3 的 ROI 可以和前面的 ROIS 完全不同。
TASK3_OPEN_ROI_PICKER = False

# TASK3_ROIS：Task 3 专用 ROI 变量，命名刻意不叫 ROIS，避免覆盖前面任务。
# None 且不打开 picker 时，默认复用初始 ROIS。
TASK3_ROIS = None

# TASK3_DEFAULT_ROI_PREFIX：napari 新画 ROI 的显示名前缀。
TASK3_DEFAULT_ROI_PREFIX = 'Task3_ROI'

if TASK3_OPEN_ROI_PICKER:
    TASK3_ROIS = pick_rois_with_napari(
        session,
        analysis_points,
        image_channel=context['image_channel'],
        image_key=IMAGE_KEY,
        default_roi_prefix=TASK3_DEFAULT_ROI_PREFIX,
    )
elif TASK3_ROIS is None:
    TASK3_ROIS = ROIS

# 用 TASK3_ROIS 重新展开 analysis_points，得到 task3_analysis_points。
# 这一步不会修改前面 Task 1/2 使用的 analysis_points。
task3_context = initialize_simple_quant_v2_analysis(
    session_path=SESSION_PATH,
    result_root=RESULT_ROOT,
    source_table=SOURCE_TABLE,
    use_keep_only=USE_KEEP_ONLY,
...
task3_analysis_points = task3_context['analysis_points']
task3_roi_table = task3_context['roi_table']

# center_groups：定义要计算几何中心的 class group。
# 每个 group 可以包含一个或多个 class_id；这里仅计算 unweighted geometric centroid。
center_groups = {
    # 'group_A': [0],
    # 'group_B': [1, 2],
}

# center_pairs：定义要比较哪些 group 的中心位移。
# 方向约定是 group_A -> group_B，并会写入表格和图标题。
center_pairs = [
    # ('group_A', 'group_B'),
]

# TASK3_MIN_POINTS_PER_GROUP：每个 group 在某个 ROI 内至少需要多少点才视为 valid。
TASK3_MIN_POINTS_PER_GROUP = 1
display(task3_roi_table)
display(pd.DataFrame([{'group_name': key, 'class_ids': value} for key, value in center_groups.items()]))
""",
        ),
        md(
            "task3-run-md",
            """
## 16. Task 3: group centroids and displacements

这个 cell 根据 `center_groups` 计算每个 Task3 ROI 内各 group 的几何中心，然后根据 `center_pairs` 计算 group_A -> group_B 的位移、距离和角度。
""",
        ),
        code(
            "task3-run",
            """
# 1) 计算每个 Task3 ROI 内每个 group 的 unweighted geometric centroid。
group_centroid_table = compute_group_centroids_by_roi(
    task3_analysis_points,
    center_groups=center_groups,
    min_points=TASK3_MIN_POINTS_PER_GROUP,
)

# 2) 按 center_pairs 计算 group_A -> group_B 的 dx/dy/distance/angle。
group_displacement_table = compute_group_pair_displacements(
    group_centroid_table,
    center_pairs=center_pairs,
    pixel_to_nm=task3_analysis_points.attrs.get('pixel_to_nm'),
)

# 3) 将 group 配置和 summary 整理成表格，供 Excel 和 manifest 记录。
task3_group_config = pd.DataFrame([{'group_name': key, 'class_ids': ','.join(str(v) for v in value)} for key, value in center_groups.items()])
task3_summary = pd.DataFrame([
    {'metric': 'task3_roi_count', 'value': len(TASK3_ROIS)},
...
print('Task 3 reports ROI-level unweighted geometric centroids only; it is not an automatic charge-center or polarization interpretation.')
""",
        ),
        md(
            "task3-fig-md",
            """
## 17. Task 3: group center and displacement figures

这里绘制 Task3 ROI、各 class group 几何中心和 group-pair displacement arrow。箭头方向默认是 `group_A -> group_B`，只表示 ROI-level 几何中心位移。
""",
        ),
        code(
            "task3-figures",
            """
# Task 3 主图：显示 ROI、group centroid 和 displacement arrow。
# 箭头方向是 group_A -> group_B；如果 center_pairs 为空，图中只显示 centroid。
fig, ax = plot_group_centers_and_displacements(
    image,
    group_centroid_table,
    group_displacement_table,
    rois=TASK3_ROIS,
    style=FIG_STYLE,
    title='Task 3 group_A -> group_B centroid displacement',
)
figures['task3_group_center_displacement'] = fig
display(fig)
plt.close(fig)
""",
        ),
        md(
            "task3-export-md",
            """
## 18. Task 3: export Excel

将 Task 3 的 group centroid、group displacement、Task3 ROI table、group config 和 summary 写入独立 Excel 文件。
""",
        ),
        code(
            "task3-export",
            """
# Task 3 独立 Excel 导出：包含 centroid、displacement、ROI、group config 和 summary。
task3_excel = export_task3_excel(
    output_dirs,
    group_centroid_table=group_centroid_table,
    group_displacement_table=group_displacement_table,
    task3_roi_table=task3_roi_table,
    task3_group_config=task3_group_config,
    task3_summary=task3_summary,
)
excel_exports['task3'] = task3_excel
display(task3_excel)
""",
        ),
        md(
            "final-export-md",
            """
## 19. Final export: tables / figures / configs / manifest
...
notebook02_configs = {
    'notebook02_config': {
        ...
        'task3_note': 'unweighted ROI-level geometric centroids only',
    }
}
